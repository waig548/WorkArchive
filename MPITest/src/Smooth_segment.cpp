...
/*  variables ended with "_intercomm" are essentially copies of 
    variables of the same name with slight modifications 
    to comply with some mechanics of MPI
*/
...
RGBTRIPLE *BMPData_intercomm = NULL;    
RGBTRIPLE *TMPData_intercomm = NULL;    
RGBTRIPLE **RECData = NULL;             // buffer for segments to be processed
RGBTRIPLE **RETData = NULL;             // buffer for processed segments
RGBTRIPLE *RECData_intercomm = NULL;
RGBTRIPLE *RETData_intercomm = NULL;
...
int main(int argc, char *argv[]) {
    ...
    if(id == 0){
        ...
        BMPData_intercomm = alloc_memory_linear(bmpInfo.biHeight, bmpInfo.biWidth);
        TMPData_intercomm = alloc_memory_linear(bmpInfo.biHeight + 2, bmpInfo.biWidth);
        ...
        // pack TMPData into linear form
        for (int i = 0; i < bmpInfo.biHeight+2; i++)
            for (int j = 0; j < bmpInfo.biWidth; j++)
                TMPData_intercomm[i*bmpInfo.biWidth+j] = TMPData[i][j];
    }
    ...
    // for transmission purposes
    RECData_intercomm = alloc_memory_linear(length + 2, biWidth);
    RETData_intercomm = alloc_memory_linear(length, biWidth);

    // scatter image base
    // due to complications, it distributes the data in linear form then unpack them into matrices, logged
    if(!MPI_Scatterv(TMPData_intercomm, sendCounts_intercomm, displacements_intercomm, rgb_triple, RECData_intercomm, (length + 2)*biWidth, rgb_triple, 0, MPI_COMM_WORLD))
        cout << "id: " << id << ": base chunk received" << endl;
    for (int i = 0; i < length+2; i++)
        for (int j = 0; j < biWidth; j++)
            RECData[i][j] = RECData_intercomm[i*biWidth+j];
    // one time thing
    free(RECData_intercomm);
    ...
    for(int count = 0; count < NSmooth; count ++){
        ...
        for(int i = 1; i < length+1; i++)
            for(int j = 0; j < biWidth; j++){
                ...
                /*********************************************************/
                RETData[i][j].rgbBlue = double(RECData[i][j].rgbBlue + RECData[Top][j].rgbBlue + RECData[Top][Left].rgbBlue + RECData[Top][Right].rgbBlue + RECData[Down][j].rgbBlue + RECData[Down][Left].rgbBlue + RECData[Down][Right].rgbBlue + RECData[i][Left].rgbBlue + RECData[i][Right].rgbBlue) / 9 + 0.5;
                RETData[i][j].rgbGreen = double(RECData[i][j].rgbGreen + RECData[Top][j].rgbGreen + RECData[Top][Left].rgbGreen + RECData[Top][Right].rgbGreen + RECData[Down][j].rgbGreen + RECData[Down][Left].rgbGreen + RECData[Down][Right].rgbGreen + RECData[i][Left].rgbGreen + RECData[i][Right].rgbGreen) / 9 + 0.5;
                RETData[i][j].rgbRed = double(RECData[i][j].rgbRed + RECData[Top][j].rgbRed + RECData[Top][Left].rgbRed + RECData[Top][Right].rgbRed + RECData[Down][j].rgbRed + RECData[Down][Left].rgbRed + RECData[Down][Right].rgbRed + RECData[i][Left].rgbRed + RECData[i][Right].rgbRed) / 9 + 0.5;
            }
    ...
    // pack RETData into linear form
    for (int i = 0; i < length; i++)
        for (int j = 0; j < biWidth; j++)
            RETData_intercomm[i*biWidth+j] = RETData[i+1][j];

    MPI_Barrier(MPI_COMM_WORLD);
    // collect processed image
    MPI_Gatherv(RETData_intercomm, length*biWidth, rgb_triple, BMPData_intercomm, receiveCounts_intercomm, displacements_intercomm, rgb_triple, 0, MPI_COMM_WORLD);
    // one time thing
    free(RETData_intercomm);
    ...
    if(id == 0){
        ...
        // unpack collected data to BMPData
        for (int i = 0; i < bmpInfo.biHeight; i++)
            for (int j = 0; j < bmpInfo.biWidth; j++)
                BMPData[i][j] = BMPData_intercomm[i*biWidth+j];
        ...
        // free pointers?
        free(BMPData);
        free(BMPData_intercomm);
        free(TMPData);
        free(TMPData_intercomm);
        free(BMPSaveData);
        free(sendCounts);
        free(displacements);
